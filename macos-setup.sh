...
# installed after getting inspired from my brewfile
brew "ack"
brew "make"
...
install_brews_from_brewfile() {
    echo ""
    echo "------------===============------------"
    echo "Installing remaining brews and casks from brewfile"
    echo "------------===============------------"
    echo "Do you want to install remaining brews and casks from brewfile? Type y for yes"
    read -r -p "**** waiting for input ****    "
    if [[ $REPLY != "y" ]]
    then
        return
...
setup_vim_tools() {
    ...
    sh -c 'curl -fLo "${XDG_DATA_HOME:-$HOME/.local/share}"/nvim/site/autoload/plug.vim --create-dirs \
       https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim'
    echo "Installed plug.vim for nvim. Do :PlugInstall after opening nvim"
